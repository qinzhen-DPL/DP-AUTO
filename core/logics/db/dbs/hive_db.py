...
class HiveData:
    ...
    def bigint_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 18446744073709551615), HiveData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-9223372036854775808, 9223372036854775807), HiveData.DEFAULT_PLACEHOLDER

    @staticmethod
    def int_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 4294967295), HiveData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-2147483648, 2147483647), HiveData.DEFAULT_PLACEHOLDER

    @staticmethod
    def decimal_data(precision, scale, unsigned, **kwargs):
        if unsigned and scale is None:
            return random_decimal(precision, 2, 0, 1000, unsigned), HiveData.DEFAULT_PLACEHOLDER
        elif not unsigned and scale is None:
            return random_decimal(precision, 2, -1000, 1000, unsigned), HiveData.DEFAULT_PLACEHOLDER
            ...
            if int_part != 0:
                ...
                maximum = 1
            return random_decimal(precision, scale, 0, maximum, unsigned), HiveData.DEFAULT_PLACEHOLDER
        else:
            int_part = int(precision) - int(scale)
            if int_part != 0:
                maximum = int('9' * int_part)
                ...
                maximum = 1
            return random_decimal(precision, scale, -maximum, maximum, unsigned), HiveData.DEFAULT_PLACEHOLDER

    @staticmethod
    def string_data(precision, scale, unsigned, **kwargs):
        return random_str_without_punctuation(20), "{0}".format(HiveData.DEFAULT_PLACEHOLDER)


class HiveFileType(Enum):
    csv = 0
    avro = 1
    parquet = 2
    orc = 3
...
class HiveDB(DBDriver):
    ALIAS = 'HIVE'

    query = {
        'get_table_schema':
            '''
select COLUMN_NAME, TYPE_NAME from COLUMNS_V2 where CD_ID =
(select CD_ID from SDS where SD_ID =
(select SD_ID from TBLS where DB_ID =
(select DB_ID from DBS where NAME = '{db_name}') and TBL_NAME = '{table_name}')) order by INTEGER_IDX''',
        'get_all_data':
            "select * from {0}",
        'delete_table':
            "drop table {0}",
...
(select DB_ID from DBS where NAME = '{0}') and TBL_NAME = '{1}'"""
    ...
    def __init__(self, db_config: dict, table_name, file_type: HiveFileType):
        ...
        # 连接hive-mysql用于读取表schema
        self.mysql_host = db_config["mysql_ip"]
        self.mysql_port = db_config["mysql_port"]
        self.mysql_username = db_config["mysql_username"]
        self.mysql_password = db_config["mysql_password"]
        self.mysql_database = db_config["mysql_database"]
        ...
        self.mysql_db = self.__connect_mysql()
    ...
    def __connect(self):
        print('连接Hive host: {0}, port: {1}, username: {2}, database: {3}'.format(
            self.host, self.port, self.user, self.database))
        db = hive.Connection(host=self.host, port=self.port, username=self.user, database=self.database)
        return db

    def __connect_mysql(self):
        print('连接Hive-MySql数据库 host: {0}, port: {1}, user: {2}, passwd: {3}, db: {4}'.format(
            self.mysql_host, self.mysql_port, self.mysql_username, self.mysql_password, self.mysql_database))
        db = pymysql.connect(host=self.mysql_host, port=self.mysql_port, user=self.mysql_username,
                             passwd=self.mysql_password, db=self.mysql_database)
        db.ping()
        return db
    ...
    @db_call
    def execute_mysql(self, sql, fetch_all=True, sql_params=None, handle_exception=False):
        if sql_params is None:
            sql_params = []
    ...
    def get_table_info(self):
        ...
        types = self.execute_mysql(self.sql('get_table_schema').format(db_name=self.database,
    ...
    def create_table(self, table_info: DBTable):
        ...
        if (self.buckets is not None) and \
                ...
                (self.file_type == HiveFileType.orc) and \
                (self.hdfs_external_file_location is None):
            tblproperties = self.sql('set-tblproperties')
        else:
            tblproperties = ''

        # 设置文件存储类型
        if self.file_type == HiveFileType.csv:
            file_type = 'TEXTFILE'
        else:
        ...
        # 分隔符设置
        if self.file_type == HiveFileType.csv:
            settings.append(self.sql('csv_format').format(self.separator_char, self.quote_char, self.escape_char))

        # 拼接DDL
        if self.hdfs_external_file_location is not None:
            table_basic = self.sql('create_external_table').format(table_name=self.table_name, columns=',\n'.join(columns),
    ...
    def add_column(self, column_info: DBColumn):
        ...
        self.execute(self.sql('add_column').format(self.table_name, column_info.name, column_info.column_type), False)
        ...
        for _ in range(count):
            ...
            for column in table_info.columns:
                ...
                if column.name == SORTED_AND_UNIQUE_COLUMN_NAME:
                    ...
                    line_placeholders.append(HiveData.DEFAULT_PLACEHOLDER)
                # 基于数据类型随机生成符合类型、精度、标度的数据
                else:
                    random_value, placeholder = getattr(HiveData, column.data_type + '_data')(
        ...
        for condition in conditions:
            ...
            for column in table_info.columns:
                ...
                if not column.not_null and random_int(0, 9) == 0:
                    values.append(None)
                    placeholders.append("{0}={1}".format(column.name, HiveData.DEFAULT_PLACEHOLDER))
                    continue
                # 基于数据类型随机生成符合类型、精度、标度的数据
                random_value, placeholder = getattr(HiveData, column.data_type + '_data')(
                    column.precision, column.scale, column.unsigned, column_type=column.column_type)
    ...
    def is_table_exist(self):
        ...
        ret = self.execute_mysql(self.sql('table_exist').format(self.database, self.table_name))
        return len(ret) == 1
