...
class OpenGaussData:
    ...
    @staticmethod
    def int1_data(precision, scale, unsigned, **kwargs):
        return random_int(0, 255), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def int2_data(precision, scale, unsigned, **kwargs):
        return random_int(-32768, 32767), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def int4_data(precision, scale, unsigned, **kwargs):
        return random_int(-2147483648, 2147483647), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def int8_data(precision, scale, unsigned, **kwargs):
        return random_int(-9223372036854775808, 9223372036854775807), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def float4_data(precision, scale, unsigned, **kwargs):
        return random_decimal(5, 2, -1000, 1000, unsigned), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def varchar_data(precision, scale, unsigned, **kwargs):
        if precision is None or precision > 50:
            precision = 50
        return random_str_which_cn_nbytes(precision, 3), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def json_data(precision, scale, unsigned, **kwargs):
        return random_json(), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def xml_data(precision, scale, unsigned, **kwargs):
        # return random_xml(), OpenGaussData.DEFAULT_PLACEHOLDER
        return None, OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def text_data(precision, scale, unsigned, **kwargs):
        return random_str(100), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def money_data(precision, scale, unsigned, **kwargs):
        return random_decimal(9, 2, -1000000, 1000000, unsigned), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def bytea_data(precision, scale, unsigned, **kwargs):
        return random_bytes(20), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def date_data(precision, scale, unsigned, **kwargs):
        return random_date(), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def time_data(precision, scale, unsigned, **kwargs):
        ...
        return time_without_f + '.' + time_f, OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def timetz_data(precision, scale, unsigned, **kwargs):
        if precision is None or precision == 0:
            precision = 6
        return OpenGaussData.time_data(precision, scale, unsigned, **kwargs)
    ...
    def reltime_data(precision, scale, unsigned, **kwargs):
        return "@ {0} minute".format(random_int(1, 9)), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def int4range_data(precision, scale, unsigned, **kwargs):
        ...
        return '[{0},{1})'.format(int_min, int_max), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def tsrange_data(precision, scale, unsigned, **kwargs):
        ...
        if t1 < t2:
            return '[{0},{1})'.format(t1_str, t2_str), OpenGaussData.DEFAULT_PLACEHOLDER
        else:
            return '[{0},{1})'.format(t2_str, t1_str), OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def tstzrange_data(precision, scale, unsigned, **kwargs):
        return OpenGaussData.tsrange_data(precision, scale, unsigned, **kwargs)

    @staticmethod
    def point_data(precision, scale, unsigned, **kwargs):
        point = random_point()
        geom_type = "(%s, %s)" % (point[0], point[1])
        return geom_type, OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def macaddr_data(precision, scale, unsigned, **kwargs):
        return random_mac(), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def box_data(precision, scale, unsigned, **kwargs):
        ...
        return box, OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def name_data(precision, scale, unsigned, **kwargs):
        return random_str_without_punctuation(5), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def uuid_data(precision, scale, unsigned, **kwargs):
        return str(random_uuid()), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def inet_data(precision, scale, unsigned, **kwargs):
        return random_ip(), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def path_data(precision, scale, unsigned, **kwargs):
        ...
        return path, OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def circle_data(precision, scale, unsigned, **kwargs):
        ...
        return circle, OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    def lseg_data(precision, scale, unsigned, **kwargs):
        ...
        return lseg, OpenGaussData.DEFAULT_PLACEHOLDER

    @staticmethod
    def polygon_data(precision, scale, unsigned, **kwargs):
        polygon = random_polygon()
        return "((%s,%s),(%s,%s),(%s,%s),(%s,%s),(%s,%s))" % tuple(polygon), OpenGaussData.DEFAULT_PLACEHOLDER
    ...
    @staticmethod
    def unknown_data(precision, scale, unsigned, **kwargs):
        return random_str_which_cn_nbytes(10), OpenGaussData.DEFAULT_PLACEHOLDER


class OpenGaussDB(DBDriver):
    ALIAS = 'OPENGAUSS'
...
{columns}
    ...
    def __connect(self):
        print('连接OpenGauss数据库 host: {0}, port: {1}, user: {2}, passwd: {3}, db: {4}'.format(
        ...
        for _ in range(count):
            ...
            for column in table_info.columns:
                ...
                if column.name == SORTED_AND_UNIQUE_COLUMN_NAME:
                    ...
                    placeholders.append(OpenGaussData.DEFAULT_PLACEHOLDER)
                # 如果字段允许为空，则有概率插入None
                ...
                # 基于数据类型随机生成符合类型、精度、标度的数据
                    ...
                    random_value, placeholder = getattr(OpenGaussData, column.data_type + '_data')(
        ...
        for condition in conditions:
            ...
            for column in table_info.columns:
                ...
                if not column.not_null and random_int(0, 9) == 0:
                    values.append(None)
                    placeholders.append("{0}={1}".format(column.name, OpenGaussData.DEFAULT_PLACEHOLDER))
                    continue
                # 基于数据类型随机生成符合类型、精度、标度的数据
                random_value, placeholder = getattr(OpenGaussData, column.data_type + '_data')(
                    column.precision, column.scale, column.unsigned, column_type=column.column_type)
