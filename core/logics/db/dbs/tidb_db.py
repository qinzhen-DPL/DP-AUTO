...
class TidbData:
    ...
    MAX_MARK = 18446744073709551615

    @staticmethod
    def tinyint_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 255), TidbData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-128, 127), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def smallint_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 65535), TidbData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-32768, 32767), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def mediumint_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 16777215), TidbData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-8388608, 8388607), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def int_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 4294967295), TidbData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-2147483648, 2147483647), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def bigint_data(precision, scale, unsigned, **kwargs):
        if unsigned:
            return random_int(0, 18446744073709551615), TidbData.DEFAULT_PLACEHOLDER
        else:
            return random_int(-9223372036854775808, 9223372036854775807), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def float_data(precision, scale, unsigned, **kwargs):
        if scale == TidbData.MAX_MARK:
            scale = None
        if unsigned and scale is None:
            return random_decimal(precision, 2, 0, 1000, unsigned), TidbData.DEFAULT_PLACEHOLDER
        elif not unsigned and scale is None:
            return random_decimal(precision, 2, -1000, 1000, unsigned), TidbData.DEFAULT_PLACEHOLDER
            ...
            if int_part != 0:
                ...
                maximum = 1
            return random_decimal(precision, scale, 0, maximum, unsigned), TidbData.DEFAULT_PLACEHOLDER
        else:
            int_part = int(precision) - int(scale)
            if int_part != 0:
                maximum = int('9' * int_part)
                ...
                maximum = 1
            return random_decimal(precision, scale, -maximum, maximum, unsigned), TidbData.DEFAULT_PLACEHOLDER
    ...
    def char_data(precision, scale, unsigned, **kwargs):
        return random_str(precision), TidbData.DEFAULT_PLACEHOLDER
    ...
    def tinyblob_data(precision, scale, unsigned, **kwargs):
        return pymysql.Binary(random_bytes(precision)), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def blob_data(precision, scale, unsigned, **kwargs):
        return pymysql.Binary(random_bytes(100)), TidbData.DEFAULT_PLACEHOLDER
    ...
    def tinytext_data(precision, scale, unsigned, **kwargs):
        return random_str(100), TidbData.DEFAULT_PLACEHOLDER
    ...
    def date_data(precision, scale, unsigned, **kwargs):
        return random_date(), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def time_data(precision, scale, unsigned, **kwargs):
        return random_time(), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def year_data(precision, scale, unsigned, **kwargs):
        return random_year(), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def datetime_data(precision, scale, unsigned, **kwargs):
        return random_datetime(), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def timestamp_data(precision, scale, unsigned, **kwargs):
        return random_timestamp(), TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def bit_data(precision, scale, unsigned, **kwargs):
        return random_bit(precision), TidbData.DEFAULT_PLACEHOLDER
    ...
    def point_data(precision, scale, unsigned, **kwargs):
        ...
        return None, TidbData.DEFAULT_PLACEHOLDER
    ...
    def geometry_data(precision, scale, unsigned, **kwargs):
        # return TidbData.point_data(precision, scale, unsigned)
        return None, TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def geometrycollection_data(precision, scale, unsigned, **kwargs):
        # return TidbData.multipoint_data(precision, scale, unsigned)
        return None, TidbData.DEFAULT_PLACEHOLDER

    @staticmethod
    def enum_data(precision, scale, unsigned, **kwargs):
        if precision == 0:
            return '', TidbData.DEFAULT_PLACEHOLDER
        else:
            search_result = re.search(r'enum\((.*)\)', kwargs['column_type'])
            if search_result:
                sp = search_result.group(1).split(',')
                return sp[random_int(0, len(sp) - 1)][1:-1], TidbData.DEFAULT_PLACEHOLDER
            else:
                return '', TidbData.DEFAULT_PLACEHOLDER
    ...
    def json_data(precision, scale, unsigned, **kwargs):
        return random_json(), TidbData.DEFAULT_PLACEHOLDER


class TidbDB(DBDriver):
    ALIAS = 'TIDB'
...
{columns}
    ...
    def get_table_info(self):
        ...
        for each in types:
            ...
            data_type = each[3]
            column_type = each[4]
            if column_type.find('int') == -1 and \
                    column_type.find('float') == -1 and \
                    column_type.find('decimal') == -1 and \
                    column_type.find('double') == -1:
    ...
    def delete_table(self, raise_error=True):
        print('表名: {0}'.format(self.table_name))
        results = self.execute(self.sql('delete_table').format(self.table_name), False)
        return results
        ...
        for _ in range(count):
            ...
            for column in table_info.columns:
                ...
                if column.name == SORTED_AND_UNIQUE_COLUMN_NAME:
                    ...
                    placeholders.append(TidbData.DEFAULT_PLACEHOLDER)
                # 如果字段允许为空，则有概率插入None
                ...
                # 基于数据类型随机生成符合类型、精度、标度的数据
                    ...
                    random_value, placeholder = getattr(TidbData, column.data_type + '_data')(
                        column.precision, column.scale, column.unsigned, column_type=column.column_type)
                    print(column.column_type, random_value)
        ...
        for condition in conditions:
            ...
            for column in table_info.columns:
                ...
                if not column.not_null and random_int(0, 9) == 0:
                    values.append(None)
                    placeholders.append("{0}={1}".format(column.name, TidbData.DEFAULT_PLACEHOLDER))
                    continue
                # 基于数据类型随机生成符合类型、精度、标度的数据
                random_value, placeholder = getattr(TidbData, column.data_type + '_data')(
                    column.precision, column.scale, column.unsigned, column_type=column.column_type)
