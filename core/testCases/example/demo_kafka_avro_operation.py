...
class KafkaAvroOperation(TestBase):
    """Kafka-avro操作演示"""
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'kafka-avro.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'kafka-avro.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='kafka_avro',
    ...
    def test_004_delete_table(self):
        """删除表"""
        self.db.delete_table(raise_error=False)
        self.db.delete_schema_registry_subject()
