...
class OpenGaussOperation(TestBase):
    """OpenGauss操作演示"""
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'opengauss.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'opengauss.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='opengauss',
                                                       table_name=cls.table_name,
                                                       configuration=config.OPENGAUSS_SINK)
