...
class OracleOperation(TestBase):
    """Oracle操作演示"""
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'oracle.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'oracle.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='oracle',
                                                       table_name=cls.table_name,
                                                       configuration=config.ORACLE_SINK)
    ...
    def test_002_column_operation(self):
        """增加、修改、删除字段"""
        self.db.add_column(DBColumn(name='new_column_1', column_type='number', default='1', not_null=True))
        self.db.add_column(DBColumn(name='new_column_2', column_type='number', default='1', not_null=True))
    ...
    def test_004_data_operation(self):
        ...
        self.db.manual_insert_data(['c1', 'c2'], [':c1', ':c2'], [1, 1])
        self.db.manual_update_data(1, ['c1', 'c2'], [':c1', ':c2'], [2, 2])
        self.db.manual_update_data('where id = 1', ['c1', 'c2'], [':c1', ':c2'], [3, 3])
