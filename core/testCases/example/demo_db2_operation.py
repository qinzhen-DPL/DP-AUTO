...
class DB2Operation(TestBase):
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'db2.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'db2.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='db2',
                                                       table_name=cls.table_name,
                                                       configuration=config.DB2_SINK)
    ...
    def test_004_data_operation(self):
        ...
        self.db.manual_update_data('where id = 1', ['c1', 'c2'], ['?', '?'], [3, 3])
