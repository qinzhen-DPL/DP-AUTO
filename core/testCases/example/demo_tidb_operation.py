...
class TiDBOperation(TestBase):
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'tidb.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'tidb.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='tidb',
                                                       table_name=cls.table_name,
                                                       configuration=config.TiDB_SINK)
    ...
    def test_002_column_operation(self):
        ...
        self.db.update_column(DBColumn(name='new_column_2', column_type='bigint', default='22', not_null=True))
        self.db.update_column(DBColumn(name='new_column_2', column_type='bigint', default=None, not_null=False))
