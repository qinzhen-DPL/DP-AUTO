...
class FtpJsonOperation(TestBase):
    """Ftp-Json操作演示"""
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'ftp.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'ftp-json.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='ftp_json',
                                                       table_name=cls.table_name,
                                                       configuration=config.FTP_SINK)
