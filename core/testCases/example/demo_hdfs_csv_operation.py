...
class HDFSCSVOperation(TestBase):
    """HDFS-csv操作演示"""
    ...
    def setUpClass(cls):
        ...
        # 创建数据操作实例
        cls.ddl_file = os.path.join(cls.data_path, 'DDL', 'example', 'hdfs-csv.yml')
        cls.export_ddl_file = os.path.join(cls.report_path, 'hdfs-csv.yml')
        cls.table_name = 'demo_automation_001'
        cls.db = create_db_instance_with_configuration(db_name='hdfs_csv',
                                                       table_name=cls.table_name,
                                                       configuration=config.HDFS_SINK)
    ...
    def test_002_table_operation(self):
        """重命名表"""
        self.db.rename_table(self.table_name + "_new")

    @skip_depends_on('test_002_table_operation')
    def test_003_data_operation(self):
        """插入、修改、删除数据"""
        self.db.insert_data(10)
        self.db.update_data(5)
        self.db.delete_data(1)
        self.db.manual_insert_data(['c1', 'c2'], [1, 1])
        self.db.manual_update_data(1, ['c1', 'c2'], [2, 2])
