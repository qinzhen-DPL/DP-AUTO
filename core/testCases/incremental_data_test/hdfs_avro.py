...
@param.via_excel('TestData.xlsx', 'hdfs-avro',
...
class TestMySqlTasks(TestBase):
    ...
    def setUpClass(cls):
        ...
        cls.now = naming_now()
        cls.ddl_files = [os.path.join(cls.data_path, 'DDL', 'regression', 'hdfs_avro_all_types_columns.yml')]
    ...
    def test_003_compare_db(self):
        ...
        schema_fail_count, data_fail_count = \
            compare_db_data_and_schema(self.node_api, self.link_api, self.context.link_id, self.context.sink_node_id,
                                       ...
                                       order_by='dpid', unique_by='dpid')
        print(schema_fail_count)
        print(data_fail_count)

    @skip_depends_on('test_003_compare_db')
    def test_004_operate_data(self):
        """增量操作"""
        for index, table in enumerate(self.source_table_names):
            source = create_db_instance(self.source_db_type, table, True)
            source.insert_data(10)
            source.update_data(5)
            source.delete_data(5)

    @skip_depends_on('test_004_operate_data')
    def test_005_operate_data(self):
        """比较全量数据同步结果"""
        schema_fail_count, data_fail_count = \
            compare_db_data_and_schema(self.node_api, self.link_api, self.context.link_id, self.context.sink_node_id,
                                       self.context.source_tables, self.source_db_type, self.source_type,
