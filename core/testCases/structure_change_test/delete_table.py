...
class DeleteTableTasks(TestBase):
    ...
    def test_004_delete_table(self):
        ...
        source.delete_table(raise_error=True)

    @skip_depends_on('test_004_delete_table')
    def test_005_delete_table(self):
        """验证同步策略"""
        self.task_api.wait_task_status_to_be(self.context.task_id, state='SUSPEND')
        source = create_db_instance(self.source_db_type, self.context.source_table, is_source=True)
        sink = create_db_instance(self.sink_db_type, self.context.sink_table, is_source=False)
        assert source.is_table_exist() is False, '上游表未删除成功'
        assert sink.is_table_exist() is True, '下游表不应被删除'
