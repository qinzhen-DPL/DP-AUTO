...
class DeleteColumnTasks(TestBase):
    ...
    def test_004_delete_column(self):
        """上游表删除字段"""
        source = create_db_instance(self.source_db_type, self.context.source_table, is_source=True)
        source.delete_column(column_name='col41')
        source.insert_data(5)

    @skip_depends_on('test_004_delete_column')
